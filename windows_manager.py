...
def save_config(folders, excludes, filters=None, theme=None, app_folders=None,
...
def create_rounded_rectangle(width, height, radius, color, shadow_color=None, border_color=None):
    """创建圆角矩形图像，支持阴影和边框"""
    if Image is None:
        return None
    try:
        if shadow_color:
            shadow_width = 3
            img = Image.new("RGBA", (width + shadow_width * 2, height + shadow_width * 2), (0, 0, 0, 0))
            draw = ImageDraw.Draw(img)
            shadow_rgba = shadow_color + (40,) if len(shadow_color) == 3 else shadow_color
            draw.rounded_rectangle(
                [(shadow_width, shadow_width), (width + shadow_width - 1, height + shadow_width - 1)],
                radius=radius, fill=shadow_rgba
            )
            img_blur = img.filter(Image.BLUR) if hasattr(Image, 'BLUR') else img
            result = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            result.paste(img_blur, (0, 0), img_blur)
        else:
            result = Image.new("RGBA", (width, height), (0, 0, 0, 0))

        draw = ImageDraw.Draw(result)
        color_rgba = color + (255,) if len(color) == 3 else color
        draw.rounded_rectangle([(0, 0), (width - 1, height - 1)], radius=radius, fill=color_rgba)

        if border_color:
            border_rgba = border_color + (100,) if len(border_color) == 3 else border_color
            draw.rounded_rectangle([(0, 0), (width - 1, height - 1)], radius=radius, outline=border_rgba, width=1)
...
def create_button_background(width, height, color, is_hover=False, is_pressed=False):
    """创建按钮背景图像，支持hover和pressed状态"""
    if Image is None:
        return None
    try:
        ...
        color_rgba = rgb + (255,)
        draw.rounded_rectangle([(0, 0), (width - 1, height - 1)], radius=4, fill=color_rgba)
        return img
    except Exception:
        return None


def create_card_background(width, height, color, border_color=None, shadow=False):
    """创建卡片背景图像，支持边框和阴影"""
    if Image is None:
        return None
    try:
        if shadow:
            ...
            draw.rounded_rectangle(
                [(shadow_width, shadow_width), (width + shadow_width - 1, height + shadow_width - 1)],
                radius=8, fill=shadow_rgb + (25,)
            )
            result = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            result.paste(img, (-shadow_width, -shadow_width), img)
        else:
            result = Image.new("RGBA", (width, height), (0, 0, 0, 0))
        ...
        rgb = hex_to_rgb(color) if isinstance(color, str) else color
        color_rgba = rgb + (255,)
        draw.rounded_rectangle([(0, 0), (width - 1, height - 1)], radius=8, fill=color_rgba)

        if border_color:
            border_rgb = hex_to_rgb(border_color) if isinstance(border_color, str) else border_color
            draw.rounded_rectangle([(0, 0), (width - 1, height - 1)], radius=8, outline=border_rgb + (80,), width=1)
...
def style_listbox(listbox, theme):
    """为Listbox应用主题样式"""
    if not isinstance(theme, dict):
        return
    listbox.configure(
        bg=theme.get("card", "#2b2b2b"),
        fg=theme.get("text", "#ffffff"),
        selectbackground=theme.get("accent", "#0078d4"),
        selectforeground=theme.get("text", "#ffffff"),
        borderwidth=1,
        relief="solid",
        activestyle="none",
...
def style_text_widget(text_widget, theme):
    """为Text组件应用主题样式"""
    if not isinstance(theme, dict):
        return
    text_widget.configure(
        bg=theme.get("card", "#2b2b2b"),
        fg=theme.get("text", "#ffffff"),
        insertbackground=theme.get("accent", "#0078d4"),
        selectbackground=theme.get("accent", "#0078d4"),
        selectforeground=theme.get("text", "#ffffff"),
        borderwidth=1,
        relief="solid"
...
class ApplicationManager(tk.Tk):
    def __init__(self):
        ...
        self.icon_shape = desktop_settings.get("icon_shape", "square")
        if self.icon_shape not in ("square", "rounded", "circle"):
            self.icon_shape = "square"
        ...
        self.theme_name = self.config_data.get("theme", "android")
        if self.theme_name not in THEMES:
            self.theme_name = "android"
        self.build_ui()
    ...
    def build_ui(self):
        ...
        ttk.Label(toolbar, text="Windows 应用资源库", font=("Segoe UI", 18, "bold")).pack(side=tk.LEFT)
        ttk.Button(toolbar, text="设置", command=self.open_settings).pack(side=tk.RIGHT, padx=(8, 0))
        self.theme_label = ttk.Label(toolbar, text="")
        self.theme_label.pack(side=tk.RIGHT)
        self.search = tk.StringVar()
        search_box = ttk.Entry(toolbar, textvariable=self.search, width=30)
        search_box.pack(side=tk.RIGHT, padx=(8, 0))
        search_box.bind("<KeyRelease>", lambda _: self.render_apps())
        ttk.Label(toolbar, text="搜索：").pack(side=tk.RIGHT)

        self.main_pane = ttk.PanedWindow(self, orient=tk.HORIZONTAL)
        self.main_pane.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        left = ttk.LabelFrame(self.main_pane, text="扫描范围", padding=8)
        self.scan_panel = left
        self.main_pane.add(left, weight=0)
        self.folder_list = tk.Listbox(left, width=38, height=18, selectmode=tk.EXTENDED)
        self.folder_list.pack(fill=tk.BOTH, expand=True)
        self.scan_button = ttk.Button(left, text="扫描", command=self.start_scan)
        self.progress = ttk.Progressbar(left, mode="determinate", maximum=1, value=0)
        self.progress.pack(fill=tk.X, pady=(10, 0))
        self.status = ttk.Label(left, text="准备就绪", wraplength=240)
        self.status.pack(fill=tk.X, pady=(10, 0))

        right = ttk.Frame(self.main_pane)
        self.main_pane.add(right, weight=1)
        if self.theme_name == "android":
            self.main_pane.forget(left)
        header = ttk.Frame(right)
        header.pack(fill=tk.X, pady=(0, 6))
        self.count_label = ttk.Label(header, text="0 个应用")
        self.count_label.pack(side=tk.LEFT)
        self.sort_value = tk.StringVar(value="名称")
        ttk.Label(header, text="排序：").pack(side=tk.RIGHT)
        sort = ttk.Combobox(header, textvariable=self.sort_value, values=("名称", "大小", "修改时间"),
                            state="readonly", width=10)
        sort.pack(side=tk.RIGHT, padx=5)
        sort.bind("<<ComboboxSelected>>", lambda _: self.render_apps())
        self.card_canvas = tk.Canvas(right, highlightthickness=0, borderwidth=0)
        card_scroll = ttk.Scrollbar(right, orient=tk.VERTICAL, command=self.card_canvas.yview)
        ...
        self.card_frame = tk.Frame(self.card_canvas, borderwidth=0)
        ...
        self.android_page_label = ttk.Label(bottom, text="")
        self.android_page_label.pack(side=tk.LEFT)
        self.android_prev_button = ttk.Button(bottom, text="‹", width=3,
                                              command=lambda: self.change_android_page(-1))
        self.android_prev_button.pack(side=tk.RIGHT)
        self.android_next_button = ttk.Button(bottom, text="›", width=3,
                                              command=lambda: self.change_android_page(1))
        self.android_next_button.pack(side=tk.RIGHT, padx=(4, 0))
        self.dock_frame = tk.Frame(right, height=74)
        self.dock_frame.pack(fill=tk.X, pady=(6, 0))
        self.dock_frame.pack_propagate(False)
        self.apply_theme()

    def apply_theme_config(self):
        """应用主题样式配置"""
        theme = THEMES.get(self.theme_name, THEMES["android"])
        ...
        style.configure("TButton", padding=(12, 6), font=("Segoe UI", 9),
                        background=card_bg, foreground=fg, borderwidth=0)
        style.map("TButton", 
                  background=[("pressed", theme.get("button_hover", accent)), ("active", theme.get("card_hover", card_bg))],
        ...
        style.configure("TEntry", fieldbackground=card_bg, foreground=fg, borderwidth=1, relief="solid")

        self.configure(bg=bg)
        if hasattr(self, 'card_canvas'):
            self.card_canvas.configure(bg=bg, highlightthickness=0, borderwidth=0)
    ...
    def save_user_config(self):
        save_config(self.folders, self.excludes, self.filters, self.theme_name, self.app_folders,
    ...
    def start_scan(self):
        if not self.folders:
            self.status.config(text="请先添加至少一个扫描目录")
            return
    ...
    def render_apps(self):
        ...
        self.filtered_apps = [app for app in self.apps
                              if self._matches_search(app, query)
                              and not self._is_hidden(app.path)]
        if sort_key == "大小":
            self.filtered_apps.sort(key=lambda app: app.size, reverse=True)
        elif sort_key == "修改时间":
            self.filtered_apps.sort(key=lambda app: app.modified, reverse=True)
        else:
            self.filtered_apps.sort(key=lambda app: app.name.lower())
        if self.theme_name == "android":
            self._render_android_apps(query)
            return
        ...
        for folder_name in sorted(self.app_folders, key=str.casefold):
            ...
            if query and query not in folder_name.casefold() and not folder_apps:
                continue
            self._create_folder_card(display_index, folder_name)
            display_index += 1
        assigned_paths = {path for paths in self.app_folders.values() for path in paths}
        for app in self.filtered_apps:
            if app.path in assigned_paths:
                continue
            self._create_app_card(display_index, app)
            display_index += 1
        self.card_frame.update_idletasks()
        self.card_canvas.configure(scrollregion=self.card_canvas.bbox("all"))
        self.count_label.config(text=f"{len(self.filtered_apps)} / {len(self.apps)} 个应用")

    def _render_android_apps(self, query):
        folder_items = []
        for folder_name in sorted(self.app_folders, key=str.casefold):
            folder_apps = [app for app in self.filtered_apps
                            if app.path in self.app_folders[folder_name]]
            if not query or query in folder_name.casefold() or folder_apps:
    ...
    def _render_android_page(self):
        ...
        for index, (kind, value) in enumerate(page_items):
            ...
            if kind == "folder":
                self._create_android_folder(row, column, value, theme)
            else:
                self._create_android_app(row, column, value, theme)
        page_count = max(1, (len(self._android_items) + page_size - 1) // page_size)
        self.android_page_label.configure(text=f"第 {self.android_page + 1} / {page_count} 页")
        self.android_prev_button.configure(state=tk.NORMAL if self.android_page else tk.DISABLED)
        self.android_next_button.configure(state=tk.NORMAL if self.android_page < page_count - 1
                                            else tk.DISABLED)
        self._render_android_dock(theme)

    def _create_android_app(self, row, column, app, theme):
        iid = f"android-app-{row}-{column}"
        self.app_by_iid[iid] = app
        icon_size = self._android_icon_size()
        card = tk.Frame(self.card_frame, bg=theme["card"], width=130, height=180,
        ...
        if self.layout_mode == "free":
            ...
            card.grid_forget()
            card.place(x=x, y=y)
        icon = tk.Label(card, bg=theme["card"], fg=theme["accent"], width=6, height=3,
                        relief=tk.FLAT)
        icon.pack(pady=(2, 4))
        placeholder = self._placeholder_icon(app, icon_size)
        if ImageTk and placeholder:
            icon.image = ImageTk.PhotoImage(placeholder)
            icon.configure(image=icon.image)
        else:
            icon.configure(text=(app.name[:1] or "A").upper(), font=("Segoe UI", 22, "bold"))
        name = tk.Label(card, text=app.name, bg=theme["card"], fg=theme["text"],
                        font=("Segoe UI", 9), wraplength=120, justify=tk.CENTER)
        name.pack(fill=tk.X)
        badge_value = self.badges.get(app.path, "")
        badge = None
        if badge_value and badge_value not in ("0", "None"):
            badge = tk.Label(card, text=badge_value, bg="#e53935", fg="white",
                             font=("Segoe UI", 8, "bold"), padx=4, pady=1)
            badge.place(relx=1.0, x=-5, y=5, anchor="ne")
        self.card_widgets[iid] = card
        self.android_app_cards.append((card, app.path, iid))
        for widget in (card, icon, name):
            widget.bind("<Enter>", lambda _, target=card: self._set_android_card_hover(target, True))
            widget.bind("<Leave>", lambda _, target=card: self._set_android_card_hover(target, False))
            widget.bind("<Button-1>", lambda _, key=iid: self.select_card(key))
            widget.bind("<Double-1>", lambda _, key=iid: self.select_card(key, launch=True))
            widget.bind("<Button-3>", lambda event, key=iid: self.show_app_menu(event, key))
            widget.bind("<ButtonPress-1>", lambda event, path=app.path, key=iid:
                        self._android_press(event, path, key))
            widget.bind("<B1-Motion>", self._android_motion)
            widget.bind("<ButtonRelease-1>", self._android_release)
        self.after(10, lambda: self._load_card_icon(app, icon, icon_size))
    ...
    def _set_android_card_hover(self, card, hovered):
        if self.android_dragging:
            return
        color = "#333333" if hovered else THEMES["android"]["card"]
        card.configure(bg=color)
        for child in card.winfo_children():
            if child.winfo_exists():
                child.configure(bg=color)

    def _create_android_folder(self, row, column, folder_name, theme):
        iid = f"android-folder-{folder_name}"
        card = tk.Frame(self.card_frame, bg=theme["card"], width=130, height=180,
                        highlightthickness=1, highlightbackground="#3a3a3a")
        card.grid(row=row, column=column, padx=8, pady=8, sticky="nsew")
        card.grid_propagate(False)
        icon = tk.Label(card, text="📁", bg=theme["card"], fg=theme["accent"],
                        font=("Segoe UI Emoji", 38))
        icon.pack(pady=(2, 4))
        name = tk.Label(card, text=folder_name, bg=theme["card"], fg=theme["text"],
                        font=("Segoe UI", 9), wraplength=120, justify=tk.CENTER)
        name.pack(fill=tk.X)
        self.card_widgets[iid] = card
        for widget in (card, icon, name):
            widget.bind("<Enter>", lambda _, target=card: self._set_android_card_hover(target, True))
            widget.bind("<Leave>", lambda _, target=card: self._set_android_card_hover(target, False))
            widget.bind("<Double-1>", lambda _, value=folder_name: self.open_folder(value))
            widget.bind("<ButtonRelease-1>", lambda _: self._drop_on_folder(folder_name))

    def _render_android_dock(self, theme):
        ...
        for app in dock_apps[:5]:
            button = tk.Button(self.dock_frame, text=app.name[:12], command=lambda path=app.path:
                               self.launch_path(path), bg=theme["card"], fg=theme["text"],
                               activebackground="#005a9e", activeforeground="white",
                               relief=tk.FLAT, padx=10)
            button.bind("<Enter>", lambda event, item=button: item.configure(bg="#333333"))
            button.bind("<Leave>", lambda event, item=button: item.configure(bg=theme["card"]))
    ...
    def _android_press(self, event, path, iid):
        ...
        self.android_dragging = False
        self.android_drag_target = None
        card = self.card_widgets.get(iid)
        if card is not None:
    ...
    def _start_android_drag(self, path, iid):
        ...
        if card is not None:
            ...
            card.configure(highlightbackground=THEMES["android"]["accent"],
                           highlightthickness=2, cursor="hand2")

    def _android_motion(self, event):
        if not self.android_dragging:
            return
        card = self.card_widgets.get(self.android_drag_iid)
        if card is not None:
        ...
        if target == self.android_drag_target:
            return
        self._clear_android_drag_target()
    ...
    def _android_release(self, event):
        ...
        if self.android_dragging:
            ...
            if target and target != source:
                ...
                card = self.card_widgets.get(source_iid)
                if card is not None:
    ...
    def _clear_android_drag_target(self):
        for card, path, _ in self.android_app_cards:
    ...
    def _create_app_card(self, index, app):
        theme = THEMES[self.theme_name]
        iid = f"app-{index}"
        self.app_by_iid[iid] = app
        if index % 5 == 0:
            self.current_card_row = tk.Frame(self.card_frame, bg=theme["background"])
            self.current_card_row.pack(fill=tk.X, anchor=tk.W)

        card_width = 130
        card_height = 180
        icon_size = 80

        card = tk.Frame(self.current_card_row, bg=theme["background"], width=card_width, height=card_height)
        card.pack(side=tk.LEFT, padx=8, pady=8, anchor=tk.N)
        card.pack_propagate(False)
        self.card_widgets[iid] = card

        inner_card = tk.Frame(card, bg=theme["card"], highlightthickness=1,
                             highlightbackground=theme.get("border", "#3a3a3a"),
                             padx=8, pady=8, width=card_width, height=card_height)
        inner_card.pack(fill=tk.BOTH, expand=True)
        inner_card.pack_propagate(False)

        icon_label = tk.Label(inner_card, bg=theme["card"])
        icon_label.pack(pady=(0, 8))
        placeholder = self._placeholder_icon(app, icon_size)
        if ImageTk and placeholder:
            icon_label.image = ImageTk.PhotoImage(placeholder)
            icon_label.configure(image=icon_label.image)
        else:
            icon_label.configure(text="APP", font=("Segoe UI", 24, "bold"), fg=theme["accent"])

        name_label = tk.Label(inner_card, text=app.name, bg=theme["card"], fg=theme["text"],
                              font=("Segoe UI", 9, ""), wraplength=120, justify=tk.CENTER)
        name_label.pack(fill=tk.X)

        def on_enter(e):
            inner_card.config(bg=theme.get("card_hover", theme["card"]))
            name_label.config(bg=theme.get("card_hover", theme["card"]))
            icon_label.config(bg=theme.get("card_hover", theme["card"]))

        def on_leave(e):
            inner_card.config(bg=theme["card"])
            name_label.config(bg=theme["card"])
            icon_label.config(bg=theme["card"])

        for widget in (card, inner_card, icon_label, name_label):
            widget.bind("<Button-1>", lambda _, key=iid: self.select_card(key))
            widget.bind("<Double-1>", lambda _, key=iid: self.select_card(key, launch=True))
            widget.bind("<Button-3>", lambda event, key=iid: self.show_app_menu(event, key))
        ...
        self.after(10, lambda: self._load_card_icon(app, icon_label, icon_size))

    def _create_folder_card(self, index, folder_name):
        theme = THEMES[self.theme_name]
        ...
        icon_label = tk.Label(inner_card, text="📁", bg=theme["card"], fg=theme["accent"],
                              font=("Segoe UI Emoji", 48))
        icon_label.pack(expand=True)

        name_label = tk.Label(inner_card, text=folder_name, bg=theme["card"], fg=theme["text"],
                              font=("Segoe UI", 9), wraplength=114, justify=tk.CENTER)
        name_label.pack(fill=tk.X)
        ...
        for widget in (card, inner_card, icon_label, name_label):
            widget.bind("<Double-1>", lambda _, name=folder_name: self.open_folder(name))
            widget.bind("<Enter>", on_enter)
            widget.bind("<Leave>", on_leave)
    ...
    def pin_to_dock(self, path):
        if path not in self.dock_paths:
            ...
            self.save_user_config()
            self._render_android_dock(THEMES["android"])
    ...
    def open_folder(self, folder_name):
        ...
        canvas = tk.Canvas(dialog, highlightthickness=0, bg=THEMES["android"]["background"])
        scrollbar = ttk.Scrollbar(dialog, orient=tk.VERTICAL, command=canvas.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(12, 0), pady=(0, 12))
        content = tk.Frame(canvas, bg=THEMES["android"]["background"])
        ...
        def render_folder(*_):
            ...
            for index, app in enumerate(apps):
                ...
                card = tk.Frame(content, bg=THEMES["android"]["card"], width=130, height=145,
                                highlightthickness=1, highlightbackground="#dfe3e8")
                card.grid(row=row, column=column, padx=8, pady=8)
                card.grid_propagate(False)
                icon = tk.Label(card, bg=THEMES["android"]["card"])
                icon.pack(pady=(8, 4))
                placeholder = self._placeholder_icon(app, 58)
                if ImageTk and placeholder:
                    icon.image = ImageTk.PhotoImage(placeholder)
                    icon.configure(image=icon.image)
                else:
                    icon.configure(text="APP", fg=THEMES["android"]["accent"])
                label = tk.Label(card, text=app.name, bg=THEMES["android"]["card"],
                                 fg=THEMES["android"]["text"], wraplength=115)
                label.pack(fill=tk.X)
                for widget in (card, icon, label):
                    widget.bind("<Double-1>", lambda _, path=app.path: self.launch_path(path))
                    widget.bind("<Button-3>", lambda event, path=app.path:
                                self.remove_from_folder_from_dialog(folder_name, path, dialog))
                self.after(10, lambda app=app, icon=icon: self._load_card_icon(app, icon, 58))
        ...
        ttk.Button(dialog, text="保存名称", command=rename_folder).pack(pady=(0, 10))
    ...
    def _placeholder_icon(self, app, size):
        if Image is None:
            return None
        theme = THEMES[self.theme_name]
        image = Image.new("RGBA", (size, size), theme["accent"])
        if ImageDraw:
    ...
    def _shape_icon_image(self, image):
        if Image is None or self.icon_shape == "square":
            return image
        mask = Image.new("L", image.size, 0)
        draw = ImageDraw.Draw(mask)
        bounds = (0, 0, image.width - 1, image.height - 1)
        if self.icon_shape == "circle":
            draw.ellipse(bounds, fill=255)
        else:
            draw.rounded_rectangle(bounds, radius=max(8, image.width // 5), fill=255)
        image = image.copy()
        image.putalpha(mask)
        return image

    def _load_card_icon(self, app, icon_label, size=96):
        ...
        if image is None:
            return
        if ImageTk:
            icon_label.image = ImageTk.PhotoImage(self._shape_icon_image(image))
            icon_label.configure(image=icon_label.image)
    ...
    def select_card(self, iid, launch=False):
        ...
        for card_iid, card in self.card_widgets.items():
            selected = card_iid == iid
            card.configure(highlightbackground=THEMES[self.theme_name]["accent"] if selected else "#3a3a3a",
    ...
    def _resize_card_frame(self, event):
        ...
        if (self.theme_name == "android" and hasattr(self, "_android_items")
    ...
    def toggle_theme(self):
        theme_order = tuple(sorted(THEMES.keys()))
        self.theme_name = theme_order[(theme_order.index(self.theme_name) + 1) % len(theme_order)]
    ...
    def apply_theme(self):
        ...
        if hasattr(self, 'card_canvas'):
            self.card_canvas.configure(bg=theme["background"])
        if hasattr(self, 'card_frame'):
            self.card_frame.configure(bg=theme["background"])
        if hasattr(self, 'dock_frame'):
            self.dock_frame.configure(bg=theme["background"])
        if hasattr(self, 'folder_list'):
            style_listbox(self.folder_list, theme)
        if hasattr(self, 'theme_label'):
            self.theme_label.configure(text=f"当前：{theme['name']}")

        if self.theme_name == "android":
            if str(self.scan_panel) in self.main_pane.panes():
                self.main_pane.forget(self.scan_panel)
            if hasattr(self, 'android_page_label'):
                self.android_page_label.pack(side=tk.LEFT)
            if hasattr(self, 'android_prev_button'):
                self.android_prev_button.pack(side=tk.RIGHT)
            if hasattr(self, 'android_next_button'):
                self.android_next_button.pack(side=tk.RIGHT, padx=(4, 0))
            if hasattr(self, 'dock_frame'):
                self.dock_frame.pack(fill=tk.X, pady=(6, 0))
        else:
            if str(self.scan_panel) not in self.main_pane.panes():
                self.main_pane.insert(0, self.scan_panel, weight=0)
            if hasattr(self, 'android_page_label'):
                self.android_page_label.pack_forget()
            if hasattr(self, 'android_prev_button'):
                self.android_prev_button.pack_forget()
            if hasattr(self, 'android_next_button'):
                self.android_next_button.pack_forget()
            if hasattr(self, 'dock_frame'):
    ...
    def _app_values(app):
        return (
            app.name, app.path, app.size_text,
            "完整" if app.metadata.has_version_metadata else "缺失",
            app.metadata.subsystem_text,
            "可提取" if app.metadata.has_icon else "无",
            f"{app.metadata.confidence_score}%",
        )
    ...
    def open_settings(self):
        ...
        directory_list = tk.Listbox(dialog, height=5)
        ...
        style_listbox(directory_list, theme)
        ...
        folder_list = tk.Listbox(dialog, height=5)
        folder_list.pack(fill=tk.X, padx=14)
        style_listbox(folder_list, theme)
        ...
        denylist = tk.Text(dialog, height=4)
        denylist.pack(fill=tk.X, padx=14)
        style_text_widget(denylist, theme)
        denylist.insert("1.0", "\n".join(self.filters["denylist"]))

        theme_row = ttk.Frame(dialog)
        theme_row.pack(fill=tk.X, padx=14, pady=8)
        ttk.Label(theme_row, text="界面风格：").pack(side=tk.LEFT)
        theme_var = tk.StringVar(value=self.theme_name)
        theme_options = tuple(sorted(THEMES.keys()))
        ttk.Combobox(theme_row, textvariable=theme_var, state="readonly",
                     values=theme_options, width=15).pack(side=tk.LEFT, padx=6)

        desktop_row = ttk.Frame(dialog)
        desktop_row.pack(fill=tk.X, padx=14, pady=(0, 8))
        ...
        def save_settings(scan=False):
            ...
            self.theme_name = theme_var.get() if theme_var.get() in THEMES else "android"
        ...
        ttk.Button(actions, text="保存", command=lambda: save_settings(False)).pack(side=tk.RIGHT)
        ttk.Button(actions, text="保存并扫描", command=lambda: save_settings(True)).pack(side=tk.RIGHT, padx=6)
    ...
    def open_filter_settings(self):
        ...
        dialog.transient(self)
        dialog.grab_set()
        size_row = ttk.Frame(dialog)
        size_row.pack(fill=tk.X, padx=14, pady=8)
        ttk.Label(size_row, text="最小 EXE 体积（KB）：").pack(side=tk.LEFT)
        min_size = tk.StringVar(value=str(max(0, self.filters["min_size"] // 1024)))
        ttk.Entry(size_row, textvariable=min_size, width=10).pack(side=tk.LEFT, padx=8)
        ttk.Label(dialog, text="额外黑名单（每行一个文件名片段）：").pack(anchor=tk.W, padx=14, pady=(8, 2))
        denylist = tk.Text(dialog, height=5, width=64)
        denylist.pack(fill=tk.X, padx=14)
        denylist.insert("1.0", "\n".join(self.filters["denylist"]))
        ...
        def save_settings():
            try:
                ...
                messagebox.showerror("设置错误", "最小体积必须是整数 KB")
                return
            self.filters = {
                "gui_only": True,
                ...
                "allowlist": [],
                "denylist": [line.strip() for line in denylist.get("1.0", tk.END).splitlines() if line.strip()],
            }
            self.save_user_config()
            dialog.destroy()
            self.start_scan()

        ttk.Button(dialog, text="保存并重新扫描", command=save_settings).pack(pady=12)
